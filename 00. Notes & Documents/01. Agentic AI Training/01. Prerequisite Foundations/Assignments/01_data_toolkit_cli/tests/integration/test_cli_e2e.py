...
# test_default_tasks_file_exists_and_has_twelve_tasks - test that the default tasks file exists and has twelve tasks
def test_default_tasks_file_exists_and_has_twelve_tasks() -> None:
    tasks = load_tasks(str(DEFAULT_TASKS_FILE))
    assert len(tasks) >= 12

# test_summary_against_committed_data - test that the summary command prints the correct output against the committed data
...
# test_points_done_against_committed_data - test that the points command prints the correct output against the committed data
...
# test_load_bob_against_committed_data - test that the load command prints the correct output against the committed data
...
# test_tag_auth_against_committed_data - test that the tag command prints the correct output against the committed data
...
# test_project_layout_has_expected_paths - test that the project layout has the expected paths  
