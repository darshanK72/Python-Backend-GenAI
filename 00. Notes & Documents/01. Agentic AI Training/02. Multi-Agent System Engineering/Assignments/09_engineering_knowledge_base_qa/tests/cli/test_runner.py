...
def test_main_single_question(capsys) -> None:
    state = initial_state("What is trunk-based development?")
    state["answer"] = "done"
    with patch("app.cli.commands.run_query", return_value=state):
        assert main(["What is trunk-based development?"]) == 0


# test_main_reports_missing_index - test that missing FAISS index errors are surfaced
def test_main_reports_missing_index(capsys) -> None:
    with patch(
        "app.cli.commands.run_query",
        side_effect=FileNotFoundError("FAISS index not found. Run: python rebuild_index.py"),
    ):
        assert main(["What is DevOps?"]) == 1
    captured = capsys.readouterr()
    assert "rebuild_index.py" in captured.err
